...
def webhook():
    ...
    try:
        ...
        print(f"[{data['received_at']}] C={data.get('close')} | {data.get('session','?')}")
...
def get_candles():
    ...
    for c in candle_history[-n:]:
        # Gebruik received_at als timestamp; cast naar 'YYYY-MM-DD HH:MM' string
        # zoals parseCSV() in master v1 verwacht.
        try:
